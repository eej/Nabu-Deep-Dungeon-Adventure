...
inputhex:
        ...
        call    setwrt
        ld      hl,seedtext
        ld      bc,VDP0 + (seedtextend-seedtext)*256
1:      ld      a,(hl)
        add     a,127
        out     (VDP0),a
        inc     hl
...
entry:
        ...
        ld	hl,MUSIC1-100			; hl <- initial address of module - 100
        call	PT3_INIT			; Inits PT3 player
...
phit:
        ...
        call hitManAni  ; show blood

         ld a,(ix+HITPOINTS)                ; man hit looses hp
         sub    1 							; not dec a, it does not support daa
         daa
         ld (ix+HITPOINTS),a
         jp nz,_monblocked
	   ...
	   ; you die
         ...
         push    ix
         push    hl
         ld	hl,MUSIC4-100			       ; hl <- initial address of module - 100
         call	PT3_INIT			       ; Inits PT3 player
         pop     hl
         pop     ix
...
1:
         ...
         /*call vWait
         ld a,8
         call keydown ; getin for newgame
         inc a
         jr z,1b*/
         halt
         call getKey
...
1:       halt
         ...
         jr nz,waitkey               ; remve for real time!!
         ld h, 4
         jr _keyfound
...
; lo schermo � MAPW colonne x MAPH righe
...
checkamulet:
         ...
         ld     hl,MUSIC6-100			; hl <- initial address of module - 100
         call   PT3_INIT			; Inits PT3 player
...
getamulet:
		...
		ld	hl,MUSIC3-100			; hl <- initial address of module - 100
		call	PT3_INIT			; Inits PT3 player
        pop    hl
        pop     ix
...
;        call 05Fh       ; screen 2
        ...
        ld a, 0xff ; tripple CT starting at 0x2000
        out VDP1, a
        ld a, 0x83 
        out VDP1, a

        ld a, 0x03 ; tripple PT starting at 0x0
        out VDP1, a
        ld a, 0x84
        out VDP1, a

introloop:
        ld bc,0xE201 ; 0xe2 to register 1: 
        call wrtvdp
...
; Print message
        ...
        ; wait  seq == 4
1:      call vWait            ; wait for vblank
        call keytest    ; returns to outer function from here

        call    seq     ; music timing I guess?
        ld  a,3         ; uhm, a <- 3
        cp  l           ; compare a to 1?
        jr  nc,1b       ; loop if no carry bit
...
keytest:        
         call _joy
         and    00010000B       ; fire button down
         jr     z,2f
         ;ret ; HACK skip keyboard test

        ;ld a,8
        ;call keydown ; getin keytest
        ;inc a
        ;ret z
...
foundamulet
        call    disscr
        ld	hl,MUSIC2-100			; hl <- initial address of module - 100
        call	PT3_INIT			; Inits PT3 player
...
; HACK: disable same broken stuff that regular game start has disabled
        ;call _nosprt
        call    drawlogo
